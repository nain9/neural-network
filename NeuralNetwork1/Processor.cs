...
namespace NeuralNetwork1
{
    ...
    internal class MagicEye
    {
        ...
        public bool ProcessImage(Bitmap bitmap)
        {
            ...
            //  Можно было, конечно, и не кидаться эксепшенами в истерике, но идите и купите себе нормальную камеру!
            ...
            int cropX = Math.Max(0, (bitmap.Width - bitmap.Height) / 2 + settings.left + settings.border);
            int cropY = Math.Max(0, settings.top + settings.border);
            int cropWidth = Math.Min(side, bitmap.Width - cropX);
            int cropHeight = Math.Min(side, bitmap.Height - cropY);
            ...
            AForge.Imaging.Filters.BradleyLocalThresholding thresholdFilter = new AForge.Imaging.Filters.BradleyLocalThresholding();
            thresholdFilter.PixelBrightnessDifferenceLimit = settings.differenceLim;
            thresholdFilter.ApplyInPlace(uProcessed);
            
            /*AForge.Imaging.Filters.RotateBilinear rotateFilter = new AForge.Imaging.Filters.RotateBilinear(random.Next(-5, 6))
            {
                FillColor = Color.White
            };
            uProcessed = rotateFilter.Apply(uProcessed);*/
            ...
            {
                scaleFilter = new AForge.Imaging.Filters.ResizeBilinear(100, 100);
                uProcessed = scaleFilter.Apply(uProcessed);
            }
            //  Получить значения сенсоров из обработанного изображения размера 200x200
